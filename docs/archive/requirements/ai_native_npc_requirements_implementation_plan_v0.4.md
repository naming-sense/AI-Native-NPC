...
- 문서 버전: **v0.4**
- Schema 상태: **2.0.0 Freeze Candidate**
- 개정일: 2026-07-26
...
최신 리뷰의 9개 지적은 모두 타당하다. 특히 다음 항목은 문구 보완이 아니라 구현 계약의 정합성 문제였다.

1. 일반 Target 16개와 `NoTarget`을 분리하지 않아 후보 상한이 흔들린 문제
2. Runtime 식별자와 모델 Feature가 섞인 문제
3. Goal Revision만 있고 실제 Goal Arbitration이 빠진 문제
4. Candidate identity hash와 Post-process version 책임이 섞인 문제
5. 수초 동안 실행되는 Skill 전체를 원자 트랜잭션으로 오해할 수 있었던 문제
6. Hidden Information 규칙이 일부 Target Kind와 실행 단계에만 정의된 문제
7. Float까지 byte-identical로 요구한 비현실적인 parity 조건
8. KPI의 분모·margin·CI·Baseline version이 비어 있던 문제
9. v0.3/v0.4 명칭과 문서 병합 정책의 충돌

v0.4는 다음 선택을 고정한다.

- **일반 Target 16개 + 고정 NoTarget 1개 = 총 17 Slot**
- **Skill 16개 × Target Slot 17개 = Candidate 272개**
- Runtime `FTargetHandle`과 모델 입력 `FTargetFeatures` 완전 분리
- Candidate Hash, Post-process Version, Calibration Version, Decision Contract Hash 분리
- 원자적 범위는 `Validate + Reserve + StartCommit`까지만
- V1은 고정 패딩 Tensor를 사용하고 ragged batch를 사용하지 않음
- 모델 입력 Float는 tolerance 비교, Hash 입력은 discrete byte-identical 비교
- 최종 문서명은 v0.4 Requirements이며 Appendix A~E가 동일 문서에 포함됨
...
## 3.6 Quota 소비와 Non-mandatory 선정 Key

- Mandatory Target은 먼저 16개 일반 slot을 소비한다.
- category별 남은 quota는 `max(0, configured quota - mandatory count of category)`다.
- Mandatory가 category quota를 넘더라도 보존하며, 해당 category의 non-mandatory quota만 0이 된다.
- quota fill 후 남은 전체 slot은 round-robin backfill한다.

정책 선호 점수가 아니라 정보 보존과 실행 가능성만 사용한다.

| Category | 정렬 Key, 앞 항목 우선 |
|---|---|
| Entity | visible_now desc → confidence desc → age asc → distance asc → canonical handle asc |
| Sound | event_priority desc → confidence desc → age asc → loudness desc → canonical handle asc |
| Cover | availability desc → generation_valid desc → path_reachable desc → distance asc → canonical handle asc |
| SmartObject | availability desc → generation_valid desc → distance asc → canonical handle asc |
| PositionLike | goal_owned desc → confidence desc → age asc → distance asc → kind ID asc → canonical handle asc |

Quota 미달로 빈 slot이 생기면 다음 category 순서로 한 개씩 round-robin backfill한다.
...
`ContinueCurrentAction`은 일반 Skill이 아니라 현재 실행을 유지하는 control candidate다.

- 실행 중 Skill/Target이 있으면 정확히 하나의 Continue candidate만 valid다.
- 현재 Target이 slot에 있으면 해당 slot을 사용한다.
- 현재 Target이 없으면 `NoTarget` slot 16을 사용한다.
- 현재 실행 중인 동일 Skill/동일 Target의 일반 candidate는 mask한다.
- Skill이 종료된 상태에서는 Continue candidate를 mask한다.
- 따라서 “Continue”와 “같은 Skill/Target을 다시 Start”하는 중복 의미가 없다.
...
### Factorized Candidate Scorer

기존의 후보별 `288→128→64→1` Dense Head는 272개 후보에서 불필요한 MAC를 만든다. v0.4는 factorized scorer를 고정한다.

```text
q = Wq(h)                         # 64, decision당 1회
s = SkillEmbedding[skill_id]      # 64
t = Wt(TargetEmbedding[slot])     # 64
p = Wp(pair_features[16])         # 64
k = LayerNorm(s + t + p)
RawScore = dot(q, k) / 8 + skill_bias + target_kind_bias
```

Candidate Parameter Proposal:

```text
params = sigmoid(Wparam(k))  # duration, speed, preferred distance, intensity
```

이 구조의 candidate scoring 부분은 대략 0.35M MAC/decision 수준이며 전체 encoder를 포함한 목표는 1.0M MAC/decision 이하이다. 실제 값은 Export된 ONNX profiler로 측정한다.
...
- unknown enum/schema/version이면 `OOD=1`
- 그 외 Tactical Context 128의 Mahalanobis distance `d` 사용
...
| 값 | 포함 내용 |
|---|---|
| `candidate_set_hash` | Target slot handle/order와 272 candidate mask |
...
수초간 실행되는 Skill 전체가 transaction인 것은 아니다.

```text
Worker/Read-only
  BuildExecutionPlan

Server Game Thread, short atomic commit
  Validate request/goal/target
  → Validate Skill preconditions
  → Reserve/transfer all resources
  → StartCommit pending Skill
  → success이면 active Skill swap
  → failure이면 rollback
...
1. `decision_id`, deadline, candidate hash, contract hash 검증
2. NPC/Target generation, Goal revision 검증
3. Skill의 허용 정보 범위로 `ValidateAtCommit`
4. Resource Handle을 canonical byte order로 정렬해 원자 예약
5. `StartCommit`은 blocking 작업을 하지 않고 1ms 안에 초기 상태만 생성
6. Start 성공 후에만 기존 Skill cancel/release와 active swap을 확정
7. Start 실패 시 새 예약을 역순 rollback하고 기존 Skill을 유지
8. 부분 예약 실패도 전부 rollback

### Reservation Lease

- 기본 lease: 2.0초
- active Skill은 0.5초마다 renew
- Skill/Resource Registry가 type별 override 가능
- lease 만료, cancel, terminal에서 release
- 후보 생성 전에는 `ReservationId`가 없으며 `ResourceGeneration`과 `AvailabilityRevision`만 존재
...
| Kind | Commit에서 허용 | 금지 |
|---|---|---|
| Entity | 존재/generation/권한, 현재 허용된 Perception, 현재 LOS가 Skill에 필요하면 검증, 피해·충돌의 authoritative 실행 | Sight Lost 후 숨은 Actor Transform으로 이동/조준 Target 갱신 |
| SoundEvent | immutable event position, event TTL, event class | attributed Actor의 현재 Transform 조회 |
| LastKnownPosition | immutable snapshot position, age, confidence, authorizing Goal | subject Actor의 현재 위치·이동·생존을 이유로 snapshot 행동 취소 |
| CoverSlot | resource generation, availability revision, entry/peek 위치, reservation | 숨은 적 Actor 위치로 cover 평가 재계산 |
| SmartObject | resource generation, availability, capacity, authored/current object transform | 관계없는 hidden Actor 상태 사용 |
| Waypoint | route generation/revision, authored position, Nav availability | 숨은 Actor 위치로 waypoint를 이동시킴 |
| WorldPosition | immutable position, authorizing Goal instance/revision, TTL, world/nav bounds | 연결된 hidden Actor Transform으로 갱신 |
| NoTarget | NPC/Goal/Skill 자체 상태 | 어떤 Target Actor 조회도 금지 |
...
단일 원본은 `ai_native_npc_schema_v2_0.yaml`이다.

생성 산출물:

- `NpcPolicySchema.generated.h`
- `npc_policy_schema_generated.py`
- `NpcPolicyEnums.generated.h`
- `npc_policy_enums_generated.py`
- `npc_policy_schema_generated.md`
- ONNX constant candidate index table
- Golden Vector fixture

수동으로 C++와 Python enum/field index를 각각 편집하지 않는다.
...
- little-endian
- integer는 고정 폭
- string은 UTF-8, uint16 byte-length prefix
- bool bitset은 LSB-first
- NaN을 Hash 입력에 사용하지 않음
- unused/padding discrete field는 0
