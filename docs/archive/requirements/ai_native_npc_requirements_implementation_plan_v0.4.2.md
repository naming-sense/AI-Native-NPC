...
# 0. v0.4.2 Cross-Environment Harness Hardening 판정

최신 리뷰는 타당하다. v0.4.1은 Schema 의미 검증과 생성 코드까지 도입했지만, 규범 테스트 리포트에 컴파일러 버전과 실행시간이 포함되어 다른 OS·Compiler에서 `validate --strict`가 실패했다. 또한 C++ 증거는 문법 검사 수준이라 Python과의 canonical bytes·hash·normalizer parity를 증명하지 못했다.

v0.4.2는 다음을 실제 실행 계약으로 추가한다.
