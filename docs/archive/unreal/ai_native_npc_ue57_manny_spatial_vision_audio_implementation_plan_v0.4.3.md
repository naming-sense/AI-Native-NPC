...
- 문서 버전: **v0.4.3**
- 문서 상태: **UE 5.7 Client Implementation Profile / Schema 2.0 RC2 Semantic Hardening**
- 개정일: 2026-07-26
- 대체 문서: 기존 `ai_native_npc_ue57_manny_spatial_vision_audio_implementation_plan.md` v0.3
- 상위 기준서: `ai_native_npc_requirements_implementation_plan_v0.4.3.md`
- Tensor 단일 원본: `ai_native_npc_schema_v2_0.yaml`
- 상위 기준서 SHA-256: `3e0cb04492339465a5e76db2dd5442c3ee3a436b71a86432b24273a746369439`
- Schema YAML SHA-256: `34fcf197e424ca9d66aff45ceb2db9b17320c48cb662cda2983356b5a9a82dd8`
- Skill Registry SHA-256: `1fb67190ea8626e0e418ae6a2a1669c3bdb9f6243fbf2092ded19025d5ab5f0e`
- Goal Registry SHA-256: `b93dbea6b63578f949d4578f3db288b23967620eadf9dd9bb445f26a6c69fcc7`
- Phase 0 판정: **GO**
...
> Tensor·Enum·Padding·Normalization·Hash가 충돌하면 본문보다 `ai_native_npc_schema_v2_0.yaml`이 우선하고, Goal·Target·Commit 책임이 충돌하면 상위 v0.4.3 요구사항이 우선한다.

---

# 0. v0.4.3 Semantic Hardening 반영
...
- Candidate/Decision Hash field order·magic·endianness를 YAML에서 Python/C++로 생성
...
/AINativeNPC
  ...
  /Docs
    ai_native_npc_requirements_implementation_plan_v0.4.3.md
...
2. 상위 v0.4.3 요구사항: Goal, Target, Candidate, Commit, KPI
...
```bash
python tools/doc_harness.py release --output dist/ai_native_npc_document_harness_v0.4.3.zip
```
...
Hash에는 discrete contract만 들어간다.

```text
magic ANPCSET2
serialization_version uint16 LE
authoritative schema source SHA-256
target slot count 17
Target Handle 17개
target_mask 17 bit, LSB-first, 3 byte
candidate_mask 272 bit, LSB-first, 34 byte
```

- Target Handle: `kind:uint8 + stable_id:uint64 LE + generation:uint32 LE + revision:uint64 LE`
- NoTarget와 unused regular slot Handle은 0
- target mask 상위 unused bit는 0
- raw float, Switch Cost, score, parameter는 제외
- Response 수신 시 hash는 **pending request에 저장된 CandidateSetHash**와 먼저 비교한다.
- 그 뒤 최신 Goal/Belief/Resource 상태를 Kind별 Commit 규칙으로 검증한다.
...
Critical 8 family와 OOD 8 family 이름은 `test_taxonomy_v1.yaml`을 단일 원본으로 사용한다. Critical은 family당 최소 64 case, 총 최소 512 case다.
...
- Generator: `0.4.3`
- Contract revision: `2.0.0-rc2`
- Schema SHA-256: `34fcf197e424ca9d66aff45ceb2db9b17320c48cb662cda2983356b5a9a82dd8`
- Skill Registry SHA-256: `1fb67190ea8626e0e418ae6a2a1669c3bdb9f6243fbf2092ded19025d5ab5f0e`
- Goal Registry SHA-256: `b93dbea6b63578f949d4578f3db288b23967620eadf9dd9bb445f26a6c69fcc7`
...
| `count_max` | `8.0` |
| `schema_contract_revision` | `2.0.0-rc2` |
...
| Index | Name | Source | Unit | Normalizer | Valid range | Missing |
|---:|---|---|---|---|---|---|
...
| 17 | `current_skill_ContinueCurrentAction_reserved_zero` | control candidate is never an executing skill | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
...
| Index | Name | Source | Unit | Normalizer | Valid range | Missing |
|---:|---|---|---|---|---|---|
| 0 | `zero_0` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 1 | `zero_1` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 2 | `zero_2` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 3 | `zero_3` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 4 | `zero_4` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 5 | `zero_5` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 6 | `zero_6` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 7 | `zero_7` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 8 | `zero_8` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 9 | `zero_9` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 10 | `zero_10` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 11 | `zero_11` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 12 | `zero_12` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 13 | `zero_13` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 14 | `zero_14` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
| 15 | `zero_15` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
...
| 15 | `reserved` | must be zero | `none` | `{"type":"constant","value":0.0}` | `[0.0,0.0]` | `{"policy":"constant","value":0.0}` |
...
| 12 | `immutable_flag` | must be 1 in V1 | `bool` | `{"type":"boolean"}` | `[0.0,1.0]` | `{"occupied_required_value":1.0,"policy":"padding_zero","value":0.0}` |
